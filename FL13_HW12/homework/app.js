...
// ----  записывали массив в localStorage? И доставали его потом?
...
// // 
// listItemLink.onclick = (event) => {
//     event.preventDefault();
//     window.history.pushState("", "", ? id=${book.id}#preview);
...
// Друзі, потрібна підказка. коли заливаю в pushState #add - у мене залишається елемент location.search в url (?id=...#add). якщо видаляти(очищувати search - перезавантажується сторінка) Як його можна прибрати без перезавантаження сторінки?? нижче відображаю свій код:
// if (action === 'add' && (!uid || uid === '')) { state = '#' + action; } else if ((action === 'edit' || action === 'preview') && localStorage.getItem(uid)) { state = `?id=${uid}#${action}`; } else { state = ''; } window.history.pushState('', '', state);
...
// let title = getObj.title;


// const form = document.createElement('div');
// form.innerHTML = `<form>
// Title<br> <input placeholder="Book Title" type="text" name="title" id="title" size="30" value="${title}"/>
